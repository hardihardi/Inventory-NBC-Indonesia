...
@section('title', 'Buat Permintaan Material Baru')
...
<div class="container-fluid">
        ...
        <div class="col-md-7">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    ...
                    <li class="breadcrumb-item active">Request Baru</li>
                </ol>
            </nav>
            <h3 class="fw-bold text-dark mb-0">Permintaan Material Baru</h3>
            <p class="text-muted">Rencanakan kebutuhan bahan baku untuk efisiensi produksi.</p>
        </div>
    ...
    <form action="{{ route('inventory.production.store') }}" method="POST" id="productionForm">
        @csrf
        <div class="row g-4">
            ...
            <div class="col-xl-4 col-lg-5">
                <div class="card border-0 shadow-sm h-100 mb-4">
                    <div class="card-header bg-white py-3 border-0">
                        <h6 class="mb-0 fw-bold"><i class="fas fa-info-circle me-2 text-primary"></i>Informasi Rencana Produksi</h6>
                    </div>
                    <div class="card-body p-4 pt-0">
                        <div class="p-3 bg-light rounded-3 mb-4 border border-light-subtle">
                            <label class="form-label small fw-bold text-uppercase text-muted opacity-75 mb-1" style="font-size: 0.65rem;">Kode Permintaan (Otomatis)</label>
                            <input type="text" class="form-control bg-white fw-bold border-0 fs-5" name="code" value="{{ $code }}" readonly>
                        </div>

                        <div class="mb-4">
                            <label class="form-label required fw-bold text-primary">Produk Hasil (Output)</label>
                            <p class="small text-muted mb-2">Pilih kain atau produk jadi yang akan dihasilkan.</p>
                            <select class="form-select select2-rich" name="item_id" required>
                                ...
                                @foreach($outputItems as $item)
                                    <option value="{{ $item->id }}" 
                                        ...
                                        data-stock="{{ number_format($item->stock, 0, ',', '.') }}"
                        ...
                        <div class="mb-4 progres-target-container">
                            ...
                            <div class="input-group input-group-lg">
                                <input type="number" class="form-control fw-bold border-end-0" name="qty_planned" min="1" required placeholder="0">
                                <span class="input-group-text bg-white fw-bold text-primary border-start-0" id="output-unit-label">UNIT</span>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" class="form-control border-start-0" name="start_date" value="{{ date('Y-m-d') }}" required>
                                </div>
                            </div>
                            <div class="col-md-12 mt-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control border-start-0" name="machine_name" placeholder="Misal: KNIT-12 / FINISH-ALPHA">
                                </div>
                                <div class="form-text small">Kosongkan jika belum ditentukan.</div>
                            </div>
                        </div>

                        <div class="mb-0">
                            <label class="form-label fw-bold">Keterangan Tambahan</label>
                            <textarea class="form-control bg-light-subtle" name="notes" rows="4" placeholder="Instruksi khusus atau catatan untuk bagian gudang..."></textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm h-100">
                    ...
                    <div class="card-body p-0">
                        ...
                        <div id="empty-msg" class="text-center py-5">
                            ...
                            <p class="small text-muted italic">Klik "Tambah Material" untuk merencanakan pemakaian bahan baku.</p>
                        </div>

                        {{-- Validation Summary/Indicator --}}
                        <div id="stock-summary" class="p-4 bg-light border-top mt-auto" style="display:none;">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="fw-bold mb-1">Status Ketersediaan Material</h6>
                                    <p class="small text-muted mb-0" id="stock-status-text">Semua material tersedia di gudang.</p>
                                </div>
                                <div id="stock-indicator" class="badge bg-success px-3 py-2 rounded-pill"><i class="fas fa-check-circle me-1"></i> AMAN</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-4 pt-3 pb-5 d-flex justify-content-end gap-2">
            <button type="button" class="btn btn-link text-decoration-none text-muted" onclick="location.reload()">
                <i class="fas fa-sync-alt me-2"></i>Reset Form
            </button>
            <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-lg" id="submitBtn">
                <i class="fas fa-save me-2"></i>Ajukan Permintaan Material
            </button>
        </div>
    </form>
</div>
    ...
    $(document).ready(function() {
        ...
        function validateStock() {
            ...
            $('.material-row').each(function() {
                ...
                const unit = $(selected.element).data('unit');
            ...
            if (hasShortage) {
                $('#stock-indicator').removeClass('bg-success').addClass('bg-warning');
                ...
                $('#stock-indicator').removeClass('bg-warning').addClass('bg-success');
