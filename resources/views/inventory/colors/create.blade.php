            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white py-3">
                        <h5 class="mb-0 fw-bold text-primary"><i class="fas fa-plus me-2"></i>Tambah Warna Baru</h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('inventory.colors.store') }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="name" class="form-label fw-bold">Nama Warna <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                       id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Merah Cabai" required>
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-2">
                                    <input type="color" class="form-control form-control-color" 
                                           id="color_picker" title="Pilih Warna" style="width: 50px;">
                                    <input type="text" class="form-control @error('hex_code') is-invalid @enderror" 
                                           id="hex_code" name="hex_code" value="{{ old('hex_code', '#000000') }}" placeholder="#000000">
                                </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3" placeholder="Opsional...">{{ old('description') }}</textarea>
                            </div>

                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('inventory.colors.index') }}" class="btn btn-light border">Batal</a>
                                <button type="submit" class="btn btn-primary px-4">
                                    <i class="fas fa-save me-1"></i> Simpan Warna
                                </button>
                            </div>
                        </form>
                    </div>
