...
@section('content')
  <div class="container-fluid py-0">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <div>
        <h4 class="mb-0 text-gray-800">Buat Retur Penjualan Baru</h4>
        <p class="mb-0 text-muted" style="font-size: 0.8rem">Proses pengembalian barang dari pelanggan</p>
      </div>
      <div>
      ...
      <div class="row g-4">
          ...
          <div class="card shadow-sm mb-4 h-100">
            ...
            <div class="card-body">
                ...
                <select class="form-select select2" id="sale_id" name="sale_id">
                  ...
                  @foreach ($sales as $sale)
                    <option value="{{ $sale->id }}"
                      ...
                      {{ $sale->invoice_number }} ({{ $sale->customer->name ?? ($sale->customer_name ?? 'Umum') }})
                    </option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="mb-3">
                ...
                <textarea class="form-control" id="reason" name="reason" rows="3" required placeholder="Contoh: Barang rusak, salah ukuran, dll">{{ old('reason') }}</textarea>
              </div>

              <div class="mb-0">
                <label for="notes" class="form-label">Catatan Tambahan</label>
                <textarea class="form-control" id="notes" name="notes" rows="2" placeholder="Catatan internal">{{ old('notes') }}</textarea>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-lg-8">
              ...
              <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="returned-items-table">
                  ...
                  <tbody>
                    <!-- JS populated -->
                    <tr id="empty-row">
                      <td colspan="5" class="text-center py-4 text-muted small">Belum ada barang yang dipilih</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="card-body">
              <div class="row align-items-center">
                <div class="col-md-6 text-center text-md-start mb-3 mb-md-0">
                  ...
                  <div class="h4 mb-0 font-weight-bold text-gray-800" id="total_display">Rp 0</div>
          ...
          <div class="d-grid gap-3 d-md-flex justify-content-md-end">
            <button type="submit" class="btn btn-success btn-sm px-5 py-2 text-white shadow-sm" id="process-return-btn" disabled>
              <i class="fas fa-check-circle me-2"></i> Proses Retur Penjualan
            </button>
          </div>
        </div>
      </div>
    ...
    $(document).ready(function () {
      ...
      function updateTable() {
        ...
        if (Object.keys(returnedItems).length === 0) {
          $tbody.append('<tr id="empty-row"><td colspan="5" class="text-center py-4 text-muted small">Belum ada barang yang dipilih</td></tr>');
        } else {
        ...
        // Auto-fill refund amount if it's 0 or empty
      ...
      $('#sale_id').change(function () {
        ...
        returnedItems = {}; // Reset for new invoice
        ...
        if (details) {
          details.forEach(function(d) {
            returnedItems[d.id] = {
              ...
              quantity: d.quantity, // Default to return all
              unit: d.unit,
              max: d.quantity
            };
          });
