...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
            <div class="mb-3 mb-md-0">
                ...
                <p class="mb-0 text-muted small">
                    Dibuat: {{ $retur->created_at->isoFormat('DD MMM YYYY, HH:mm') }}
                </p>
            </div>
            <div class="d-flex gap-2 flex-wrap">
                ...
                <a href="{{ route('penjualan.retur.edit', $retur) }}" class="btn btn-warning btn-sm text-white">
                    ...
                    <div class="card-body p-0">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                ...
                                <span class="fw-bold text-primary">{{ $retur->return_number }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                <span class="text-muted"><i class="fas fa-file-invoice me-2 text-secondary"></i>Ref. Penjualan</span>
                                @if (optional($retur->sale)->exists())
                                    <a href="{{ route('penjualan.transaksi.show', $retur->sale) }}"
                                        class="fw-bold text-decoration-none">
                                        <i class="fas fa-external-link-alt me-1 small"></i>#{{ $retur->sale->invoice_number }}
                                    </a>
                                @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                ...
                                <span class="fw-semibold">{{ $retur->return_date ? $retur->return_date->isoFormat('DD MMMM YYYY') : 'N/A' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                <span class="text-muted"><i class="fas fa-user me-2 text-secondary"></i>Pelanggan</span>
                                <span class="fw-semibold">{{ optional($retur->sale)->customer_name ?? (optional(optional($retur->sale)->customer)->name ?? 'Umum') }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                <span class="text-muted"><i class="fas fa-user-check me-2 text-secondary"></i>Diproses oleh</span>
                                <span class="fw-semibold">{{ optional($retur->user)->name ?? 'N/A' }}</span>
                            </li>
                        </ul>
                    </div>

                    {{-- Alasan Retur --}}
                    @if($retur->reason || $retur->notes)
                    <div class="card-footer bg-light border-top p-3">
                        @if($retur->reason)
                        <div class="mb-2">
                            <small class="fw-semibold text-muted d-block mb-1">
                                <i class="fas fa-exclamation-circle me-1 text-warning"></i>Alasan Retur:
                            </small>
                            <p class="mb-0 text-dark">{{ $retur->reason }}</p>
                        </div>
                        @endif
                        @if($retur->notes)
                        <div>
                            <small class="fw-semibold text-muted d-block mb-1">
                                <i class="fas fa-sticky-note me-1 text-info"></i>Catatan:
                            </small>
                            <p class="mb-0 fst-italic text-secondary">{{ $retur->notes }}</p>
                        </div>
                        @endif
                    </div>
                    @endif
                ...
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-header bg-white p-3 border-bottom">
                        <h5 class="mb-0 fw-semibold">
                            ...
                            <span class="badge bg-primary ms-2">{{ $retur->items->count() }} item</span>
                        </h5>
                    </div>
                    <div class="card-body p-0">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0" id="retur-item-table">
                                        ...
                                        @foreach ($retur->items as $index => $item)
                                            <tr>
                                                ...
                                                <td data-label="Produk">
                                                    <span class="fw-semibold">{{ optional($item->item)->name ?? 'Produk Tidak Ditemukan' }}</span>
                                                ...
                                                <td data-label="Harga" class="text-end">Rp {{ number_format($item->price_per_unit, 0, ',', '.') }}</td>
                                    ...
                                    <tfoot class="table-light">
                                        <tr>
                                            <td colspan="4" class="text-end fw-bold ps-3">Total Nilai Barang:</td>
                                            <td class="text-end pe-3 fw-bold text-dark fs-5">
                                                Rp {{ number_format($retur->total_returned_amount ?? 0, 0, ',', '.') }}
                                            </td>
                                        </tr>
                                        <tr class="table-success">
                                            <td colspan="4" class="text-end fw-bold ps-3 text-success">
                                                <i class="fas fa-money-bill-wave me-1"></i>Jumlah Refund:
                                            </td>
                                            <td class="text-end pe-3 fw-bold text-success fs-5">
                                                Rp {{ number_format($retur->refund_amount ?? 0, 0, ',', '.') }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
        ...
        <div class="d-grid gap-2 d-md-none mt-4">
            ...
            <a href="{{ route('penjualan.retur.edit', $retur) }}" class="btn btn-warning text-white">
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            Swal.fire(
                                ...
                                window.location.href = "{{ route('penjualan.retur.index') }}";
                            });
                        },
                        error: function (xhr) {
